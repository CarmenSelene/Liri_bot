...
function concertApp() {
    ...
    axios.get("https://rest.bandsintown.com/artists/" + bandSearchTerm + "/events?app_id=56ef129237e7f760daabd22f3e92fe65&date=upcoming").then(
        function (response) {
            var i;
            for (i = 0; i < response.data.length -1; i++) {
                var inputDate = response.data[i].datetime;
                var convertedDate = moment(inputDate).format('MM DD YYYY');
                console.log('*****');
